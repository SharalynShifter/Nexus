...
void Gui::render() {
	...
	renderBar(1, 5, BAR_WIDTH, xpTxt, engine.player->destructible->xp,
		ai->getNextLevelXp(), TCODColor::lightViolet, TCODColor::darkViolet); 
	...
	for (Actor** it = engine.actors.begin(); it != engine.actors.end(); it++) {
		...
		if (actor->x == engine.mouse.cx && actor->y == engine.mouse.cy) {
			...
			if (actor->destructible && !actor->destructible->isDead()) {
				strcat(buf, " ");
				strcat(buf, itoa(actor->destructible->hp, buffer1, 10));
				strcat(buf, "/");
				strcat(buf, itoa(actor->destructible->maxHp, buffer2, 10));
			}
		}
	}
